...
train_name = "00.train"
train_task_fmt = "%03d"
train_neu_task_fmt = "neu.%03d"
train_red_task_fmt = "red.%03d"
...
def make_fp_neu_task_name(sys_idx, counter):
    return "task_neu." + fp_task_fmt % (sys_idx, counter)

def make_fp_red_task_name(sys_idx, counter):
    return "task_red." + fp_task_fmt % (sys_idx, counter)
...
def make_train(iter_index, jdata, mdata):
    ...
    #改为neu 与 red 路径
    init_data_prefix = jdata["init_data_prefix"]#这里获取读数据路径
    init_data_prefix = os.path.abspath(init_data_prefix)
    init_neu_data_sys_ = jdata["init_neu_data_sys"]
    init_red_data_sys_ = jdata["init_red_data_sys"]
    fp_task_min = jdata["fp_task_min"]
    model_devi_jobs = jdata["model_devi_jobs"]
    use_ele_temp = jdata.get("use_ele_temp", 0)
    training_iter0_model = jdata.get("training_iter0_model_path", [])
    training_init_model = jdata.get("training_init_model", False)#默认无
    ...
    cwd = os.getcwd()
    os.chdir(work_path)
    #将初始dataset链接到00.train/data.init
    os.symlink(os.path.abspath(init_data_prefix), "data.init")
    # link iter data
    os.mkdir("data.iters")
    os.chdir("data.iters")
    #将每一轮跑出来的iter data 链接到00.train/data.iters
    ...
    if "init_batch_size" in jdata:
        init_neu_batch_size_ = list(jdata["init_batch_size"])
        if len(init_neu_data_sys_) > len(init_neu_batch_size_):
            warnings.warn(
                "The batch sizes are not enough. Assume auto for those not spefified."
            )
            init_neu_batch_size.extend(
                ["auto" for aa in range(len(init_neu_data_sys_) - len(init_neu_batch_size_))]
            )
    else:
        init_neu_batch_size_ = ["auto" for aa in range(len(jdata["init_data_sys"]))]
    ...
    assert len(init_neu_data_sys_) <= len(init_neu_batch_size_)
    #neu
    for ii, ss in zip(init_neu_data_sys_, init_neu_batch_size_):
        sys_paths = expand_sys_str(os.path.join(init_data_prefix, ii))
        for single_sys in sys_paths:
            init_data_neu_sys.append(
                os.path.normpath(
                    os.path.join(
                        "..",
                        "data.init",
            ...
            init_neu_batch_size_.append(detect_batch_size(ss, single_sys))
    #red
    if "init_batch_size" in jdata:
        init_red_batch_size_ = list(jdata["init_batch_size"])
        if len(init_red_data_sys_) > len(init_red_batch_size_):
            warnings.warn(
                "The batch sizes are not enough. Assume auto for those not spefified."
            )
            init_red_batch_size.extend(
                ["auto" for aa in range(len(init_red_data_sys_) - len(init_red_batch_size_))]
            )
    else:
        init_red_batch_size_ = ["auto" for aa in range(len(jdata["init_data_sys"]))]
    if "sys_batch_size" in jdata:
        sys_batch_size = jdata["sys_batch_size"]
    else:
        sys_batch_size = ["auto" for aa in range(len(jdata["sys_configs"]))]
    ...
    # make sure all init_data_sys has the batch size -- for the following `zip`
    assert len(init_red_data_sys_) <= len(init_red_batch_size_)
    #red
    for ii, ss in zip(init_red_data_sys_, init_red_batch_size_):
        sys_paths = expand_sys_str(os.path.join(init_data_prefix, ii))
        for single_sys in sys_paths:
            init_data_red_sys.append(
                os.path.normpath(
                    os.path.join(
                        "..",
                        "data.init",
            ...
            init_red_batch_size.append(detect_batch_size(ss, single_sys))
    ...
    if iter_index > 0:
        ...
        for ii in range(iter_index):
            if ii == iter_index - 1:
                old_range = len(init_data_neu_sys)
            fp_path = os.path.join(make_iter_name(ii), fp_name)
            fp_data_neu_sys = glob.glob(os.path.join(fp_path, "data_neu.*"))
            ...
            for jj in fp_data_neu_sys:
                sys_idx = int(jj.split(".")[-1])#获取data iter id
                sys_paths = expand_sys_str(jj)#获得所有包含名为"type.raw"的文件的文件夹的路径字符串 完整
                nframes = 0
        ...
        for ii in range(iter_index):
            if ii == iter_index - 1:
                old_range = len(init_data_red_sys)
            fp_path = os.path.join(make_iter_name(ii), fp_name)
            fp_data_red_sys = glob.glob(os.path.join(fp_path, "data_red.*"))
            if model_devi_engine == "calypso":
                _modd_path = os.path.join(
                    make_iter_name(ii), model_devi_name, calypso_model_devi_name
                )
    ...
    if Version(mdata["deepmd_version"]) >= Version("1") and Version(
        ...
        # 1.x
        jinput["training"]["systems"] = init_data_neu_sys
        ...
        mdata["deepmd_version"]    #2.1.版本run这
    ) < Version("3"):#这强制参数systems
        # 2.x
        jinput["training"]["training_data"] = {}
        jinput["training"]["training_data"]["systems"] = init_data_neu_sys
        #jinput["training"]["training_data"]["red_systems"] = init_data_red_sys
        #这要做区分
        jinput["training"]["training_data"]["batch_size"] = 8
        jinput["model"]["type_map"] = jdata["type_map"]
        # electron temperature
        if use_ele_temp == 0:
            pass
    ...
    #创建训练文件夹并创建input.json
    for ii in range(numb_models):
        task_neu_path = os.path.join(work_path, train_neu_task_fmt % ii) #"neu.%03d"
        task_red_path = os.path.join(work_path, train_red_task_fmt % ii) #"neu.%03d"
        create_path(task_neu_path)
        create_path(task_red_path)
        #neu
        os.chdir(task_neu_path)

        if "srtab_file_path" in jdata.keys():
            shutil.copyfile(srtab_file_path, os.path.basename(srtab_file_path))

        for jj in init_data_neu_sys:
            # HDF5 path contains #
            #检查路径是否存在
            if not (
        ...
        os.chdir(cwd)
        #red
        os.chdir(task_red_path)

        if "srtab_file_path" in jdata.keys():
            shutil.copyfile(srtab_file_path, os.path.basename(srtab_file_path))

        for jj in init_data_red_sys:
            # HDF5 path contains #
            #检查路径是否存在
            if not (
                os.path.isdir(jj) if "#" not in jj else os.path.isfile(jj.split("#")[0])
        ...
        # dump the input.json
        with open(os.path.join(task_neu_path, train_input_file), "w") as neufile:#train_input_file = input.json
            json.dump(jinput, neufile, indent=4)
        kinput = copy.deepcopy(jinput)  # 使用深拷贝方法创建jinput的副本，让写入时不相互影响
        kinput["training"]["training_data"]["systems"] = init_data_red_sys
        with open(os.path.join(task_red_path, train_input_file), "w") as redfile:#train_input_file = input.json
            json.dump(kinput, redfile, indent=4)
        #input_files.append(os.path.join(task_path, train_input_file))

    # link old models
    if iter_index > 0:
        prev_iter_name = make_iter_name(iter_index - 1) #获得上一轮id
        prev_work_path = os.path.join(prev_iter_name, train_name) #获得上一层路径
        for ii in range(numb_models):
            prev_neu_task_path = os.path.join(prev_work_path, train_neu_task_fmt % ii)
            prev_red_task_path = os.path.join(prev_work_path, train_red_task_fmt % ii)
            old_neu_model_files = glob.glob(os.path.join(prev_neu_task_path, "model.ckpt*"))
            old_red_model_files = glob.glob(os.path.join(prev_red_task_path, "model.ckpt*"))
            _link_old_neu_models(work_path, old_neu_model_files, ii)
            _link_old_red_models(work_path, old_red_model_files, ii)
    else:#training_iter0_model默认为空后续执行无效
...
def _link_old_neu_models(work_path, old_model_files, ii):
    """
    link the `ii`th old model given by `old_model_files` to
    the `ii`th training task in `work_path`
    """
    task_path = os.path.join(work_path, train_neu_task_fmt % ii)
    task_old_path = os.path.join(task_path, "old")
    create_path(task_old_path)
    cwd = os.getcwd()
    for jj in old_model_files:
...
def _link_old_red_models(work_path, old_model_files, ii):
    """
    link the `ii`th old model given by `old_model_files` to
    the `ii`th training task in `work_path`
    """
    task_path = os.path.join(work_path, train_red_task_fmt % ii)
    task_old_path = os.path.join(task_path, "old")
    create_path(task_old_path)
    cwd = os.getcwd()
    for jj in old_model_files:
...
def run_train(iter_index, jdata, mdata):
    ...
    # paths
    iter_name = make_iter_name(iter_index) #这个函数就是用于快速获取iter路径的
    work_path = os.path.join(iter_name, train_name)
    # check if is copied
    copy_flag = os.path.join(work_path, "copied")
    if os.path.isfile(copy_flag):
    ...
    all_red_task = []
    for ii in range(numb_models):
        task_neu_path = os.path.join(work_path, train_neu_task_fmt % ii) #train_task_fmt = "%03d" 
        task_red_path = os.path.join(work_path, train_red_task_fmt % ii) #train_task_fmt = "%03d"
        #这好像可以直接改
        all_neu_task.append(task_neu_path)
        all_red_task.append(task_red_path)
    commands = []
    if Version(mdata["deepmd_version"]) >= Version("1") and Version(
        mdata["deepmd_version"]
    ) < Version("3"):# run this
        ...
        commands.append(command)
        if jdata.get("dp_compress", False): #配置文件中可开启compress
    ...
    #     else:
    #         run_tasks.append(ii)
    #run_tasks = [os.path.basename(ii) for ii in all_task] # get task basename for submission
    #提交任务用
    run_neu_tasks = [os.path.basename(ii) for ii in all_neu_task]
    run_red_tasks = [os.path.basename(ii) for ii in all_red_task]

    forward_files = [train_input_file] #'input.json
    if "srtab_file_path" in jdata.keys():#默认无
        forward_files.append(zbl_file)
    if training_init_model:#如有初始模型
        forward_files += [
    ...
    if jdata.get("dp_compress", False):#默认关闭压缩
        backward_files.append("frozen_model_compressed.pb")
    if not jdata.get("one_h5", False): #run
        #获取neu data 文件名
        init_neu_data_sys_ = jdata["init_neu_data_sys"]
        init_neu_data_sys = []
        #获取red data 文件名
        init_red_data_sys_ = jdata["init_red_data_sys"]
        init_red_data_sys = []        
        for ii in init_neu_data_sys_:
            init_neu_data_sys.append(os.path.join("data.init", ii)) #前面有添加软链，这里data.init = init_data_prefix
        for ii in init_red_data_sys_:
            init_red_data_sys.append(os.path.join("data.init", ii)) #前面有添加软链，这里data.init = init_data_prefix
        #储存所有neu red数据集列表
        trans_comm_data_neu = []
        trans_comm_data_red = []        
        ...
        os.chdir(work_path)
        #读取前面轮次计算获取的dataset
        fp_neu_data = glob.glob(os.path.join("data.iters", "iter.*", "02.fp", "data_neu.*")) #与fp对应，15.58已修改
        fp_red_data = glob.glob(os.path.join("data.iters", "iter.*", "02.fp", "data_red.*")) #与fp对应，15.58已修改
        #neu training comm file
        for ii in itertools.chain(init_neu_data_sys, fp_neu_data): #连成一个迭代列表
            sys_paths = expand_sys_str(ii)
            for single_sys in sys_paths:
                if "#" not in single_sys:#只上传set文件和type.raw文件
                    trans_comm_data_neu += glob.glob(os.path.join(single_sys, "set.*"))
                    trans_comm_data_neu += glob.glob(os.path.join(single_sys, "type*.raw"))
                    trans_comm_data_neu += glob.glob(os.path.join(single_sys, "nopbc"))
                else:#不执行
                    # H5 file
                    trans_comm_data.append(single_sys.split("#")[0])
        #red training comm file
        for ii in itertools.chain(init_red_data_sys, fp_red_data): #连成一个迭代列表
            sys_paths = expand_sys_str(ii)
            for single_sys in sys_paths:
                if "#" not in single_sys:#只上传set文件和type.raw文件
                    trans_comm_data_red += glob.glob(os.path.join(single_sys, "set.*"))
                    trans_comm_data_red += glob.glob(os.path.join(single_sys, "type*.raw"))
                    trans_comm_data_red += glob.glob(os.path.join(single_sys, "nopbc"))
                else:#不执行
                    # H5 file
                    trans_comm_data.append(single_sys.split("#")[0])
    else:
    ...
    #trans_comm_data = list(set(trans_comm_data)) #所有数据文件去重
    trans_comm_data_neu = list(set(trans_comm_data_neu)) #所有数据文件去重
    trans_comm_data_red = list(set(trans_comm_data_red)) #所有数据文件去重
    os.chdir(cwd)
    ...
    if Version(api_version) < Version("1.0"):
        ...
        #提交neu taining
        #改：并发提交所有任务
        run_all_tasks = run_neu_tasks + run_red_tasks  #将两个任务列表合并为一个
        trans_comm_data_all = trans_comm_data_neu + trans_comm_data_red
        submission = make_submission(
            mdata["train_machine"],
            mdata["train_resources"],
            commands=commands,
            work_path=work_path,
            run_tasks=run_all_tasks,
            group_size=train_group_size,
            forward_common_files= trans_comm_data_all,
        ...
        #     commands=commands,
        #     work_path=work_path,
        #     run_tasks=run_red_tasks,
        #     group_size=train_group_size,
        #     forward_common_files=trans_comm_data_red,
        #     forward_files=forward_files,
        #     backward_files=backward_files,
        #     outlog="train.log",
        #     errlog="train.log",
...
def post_train(iter_index, jdata, mdata):
    ...
    #neu & red
    for ii in range(numb_models):
        if not jdata.get("dp_compress", False):#默认
            model_name = "frozen_model.pb"
        else:
            model_name = "frozen_model_compressed.pb"
        task_neu_file = os.path.join(train_neu_task_fmt % ii, model_name) #train_neu_task_fmt = "neu.%03d"
        task_red_file = os.path.join(train_red_task_fmt % ii, model_name) #train_neu_task_fmt = "red.%03d"
        #neu
        neufile = os.path.join(work_path, "neu.%03d.pb" % ii)
        if os.path.isfile(neufile):
            os.remove(neufile)
        os.symlink(task_neu_file, neufile)
        #red
        redfile = os.path.join(work_path, "red.%03d.pb" % ii)
        if os.path.isfile(redfile):
            os.remove(redfile)
        os.symlink(task_red_file, redfile)  #8.9 9:50 检查链接正确
        #建立软链  这是链到training 根目录的
...
def make_model_devi(iter_index, jdata, mdata):#iter_inde from ii为迭代轮次
    # The MD engine to perform model deviation
    # Default is lammps
    model_devi_engine = jdata.get("model_devi_engine", "lammps")#md软件默认lammps

    model_devi_jobs = jdata["model_devi_jobs"]#param里拿到model devi 的运行参数
    if model_devi_engine != "calypso":#默认执行
    ...
    if "sys_configs_prefix" in jdata:#如果有prefix创建一个sys_configs
        sys_configs = []#md的模型文件一般为POSCAR
        for sys_list in jdata["sys_configs"]:
            ...
            sys_configs.append(temp_sys_list)#将所有"sys_configs_prefix"路径放入sys_configs列表
    else:
        sys_configs = jdata["sys_configs"]
    shuffle_poscar = jdata.get("shuffle_poscar", False)#shuffle_poscar默认不开启

    if model_devi_engine != "calypso":#默认运行
        cur_job = model_devi_jobs[iter_index] #获取md运行参数
        sys_idx = expand_idx(cur_job["sys_idx"]) #获取run   eg:0,1
    else:
    ...
    for idx in sys_idx:
        cur_systems = []
        ss = sys_configs[idx] #将md运行数据路径给ss
        for ii in ss:
        ...
        conf_systems.append(cur_systems)#获取到md data的绝对路径   conf_systems

    iter_name = make_iter_name(iter_index) #iter_index=ii迭代轮次
    train_path = os.path.join(iter_name, train_name) #获得train 的路径用于获取.pb
    train_path = os.path.abspath(train_path)#获取绝对路径
    models = sorted(glob.glob(os.path.join(train_path, "*.pb")))#照文件名的字母顺序进行升序排序模型文件
    work_path = os.path.join(iter_name, model_devi_name)#获取model devi 的工作路径 /iter.000000/01.model_devi
    create_path(work_path) #创建文件夹
    if model_devi_engine == "calypso":#默认不执行
    ...
    for mm in models:
        model_name = os.path.basename(mm)#获取模型名 neu.pb,red.pb
        if model_devi_engine != "calypso":#默认执行
            os.symlink(mm, os.path.join(work_path, model_name))#这是一个串联链接，源文件在train task下,定义一个软链指向train的八个模型，放在model devi work path 用于调用
        else:
            for temp_calypso_run_opt_path in calypso_run_opt_path:
    ...
    with open(os.path.join(work_path, "cur_job.json"), "w") as outfile:#
        json.dump(cur_job, outfile, indent=4)#将ur_job字典写入JSON文件

    conf_path = os.path.join(work_path, "confs")
    create_path(conf_path)#创建confs文件夹
    sys_counter = 0 #数据集个数计数器
    for ss in conf_systems:
        conf_counter = 0#数据集文件个数计数器
        for cc in ss:
            if model_devi_engine == "lammps":
                ...
                poscar_name = conf_name + ".poscar"#创建文件名变量
                lmp_name = conf_name + ".lmp"#创建文件名变量
                if shuffle_poscar:#默认不开启
                    ...
                    os.symlink(cc, os.path.join(conf_path, poscar_name))#cc为配置文件中的model devi data原文件，在confs文件夹中建立一个名为poscar的软链
                if "sys_format" in jdata:#判读有没有指认数据格式的参数
                ...
                if jdata.get("model_devi_nopbc", False):#默认无周期性
                    system.remove_pbc()
                system.to_lammps_lmp(os.path.join(conf_path, lmp_name))#转换成lammps文件
...
def _make_model_devi_native(iter_index, jdata, mdata, conf_systems): #定义input参数，准备计算
    model_devi_jobs = jdata["model_devi_jobs"]
    if iter_index >= len(model_devi_jobs):
        return False
    cur_job = model_devi_jobs[iter_index]#与前面定义相同cur_job是model_devi_jobs的任务参数
    ensemble, nsteps, trj_freq, temps, press, pka_e, dt = parse_cur_job(cur_job)#将值取出赋值给变量
    if dt is not None: #如果有则定义步长
        model_devi_dt = dt
    sys_idx = expand_idx(cur_job["sys_idx"])#又一次获取数据集id
    if len(sys_idx) != len(list(set(sys_idx))):
        raise RuntimeError("system index should be uniq")

    use_ele_temp = jdata.get("use_ele_temp", 0)#默认为0
    model_devi_dt = jdata["model_devi_dt"]#定义步长
    ...
    train_path = os.path.join(iter_name, train_name)
    train_path = os.path.abspath(train_path)
    neu_models = glob.glob(os.path.join(train_path, "neu*pb"))
    red_models = glob.glob(os.path.join(train_path, "red*pb"))
    task_model_list = []
    neu_model_list = []
    red_model_list = []
    #neu models
    for ii in neu_models:
        neu_model_list.append(os.path.join("..", os.path.basename(ii)))
    #red models
    for ii in red_models:
        red_model_list.append(os.path.join("..", os.path.basename(ii)))
    work_path = os.path.join(iter_name, model_devi_name)
        ...
        for cc in ss:
            for tt_ in temps:
                ...
                for pp in press:
                    ...
                    # dlog.info(task_path)
                    create_path(task_path)
                    model_devi_merge_traj = jdata.get("model_devi_merge_traj", False)#默认不开启
                    if not model_devi_merge_traj:
                        create_path(os.path.join(task_path, "traj"))
                    loc_conf_name = "conf.lmp"
                    os.symlink(
                        os.path.join(os.path.join(work_path, "confs"), conf_name),
                        os.path.join(task_path, loc_conf_name),
                    )#建立confs中imp,到task目录下"conf.lmp"的软链

                    #建立pb文件软链到task文件夹下
                    task_path_abs = os.path.abspath(task_path)
                    work_path_abs = os.path.abspath(work_path)
                    pb_models = sorted(glob.glob(os.path.join(work_path_abs, "*.pb"))) #model_devi目录下的 pb文件
                    for mm in pb_models:
                        model_name = os.path.basename(mm)
                        os.symlink(mm, os.path.join(task_path_abs, model_name))

                    #modified by LJS

                    #插点
                    eta = np.linspace(0, 1, 5) #插点数后续考虑变量修改
                    for num in eta:
                        num = str(num)
                        folder_name = os.path.join(task_path, num)
                        os.makedirs(folder_name, exist_ok=True) #创建文件夹
                        #创建traj文件夹
                        traj_name = os.path.join(folder_name, "traj")
                        os.makedirs(traj_name, exist_ok=True) #创建文件夹
                        #建立confs软链放到每一个文件夹中
                        folder_name = os.path.abspath(folder_name)
                        work_path_abs = os.path.abspath(work_path)
                        os.symlink(
                        os.path.join(os.path.join(work_path_abs, "confs"), conf_name),
                        os.path.join(folder_name, loc_conf_name),
                        )#建立confs中lmp,到task/num目录下"conf.lmp"的软链
                            # 创建文件
                        num = float(num)
                        if num == 0:
                            fstate=0.0001
                            ...
                            fstate=num
                        cwd_ = os.getcwd()#获取当前路径
                        os.chdir(folder_name)#task_path = os.path.join(work_path, task_name) #after modified
                        try:
                            mdata["deepmd_version"]
                        except KeyError:
                            mdata = set_version(mdata)#获取md版本
                        deepmd_version = mdata["deepmd_version"]
                        #make_fep_lammps_input属于外加的外置函数，需要添加到环境可以检索的lammps.py文件中
                        file_in = make_fep_lammps_input(
                            ensemble,
                            loc_conf_name,
                            neu_model_list,
                            red_model_list,
                        ...
                        # with open(os.path.join(folder_name, "input.lammps"), "w") as fp:
                        #      fp.write(file_in)
                        job = {}
                        ...
                        with open(os.path.join(folder_name, "job.json"), "w") as _outfile:#写入job.json文件
                    ...
                    # cwd_ = os.getcwd()#获取当前路径
                    # os.chdir(task_path)#task_path = os.path.join(work_path, task_name)
                    # try:
                    #     mdata["deepmd_version"]
                    # except KeyError:
                    #     mdata = set_version(mdata)#获取md版本
...
def run_md_model_devi(iter_index, jdata, mdata):  #model_devi运行的定义函数

    # rmdlog.info("This module has been run !")
    model_devi_exec = mdata["model_devi_command"] #"lmp -i input.lammps -v restart 0"

    model_devi_group_size = mdata["model_devi_group_size"] #"group_size": 50
    model_devi_resources = mdata["model_devi_resources"]
    use_plm = jdata.get("model_devi_plumed", False) #默认不开启
    use_plm_path = jdata.get("model_devi_plumed_path", False) #默认不开启
    model_devi_merge_traj = jdata.get("model_devi_merge_traj", False) #默认不开启

    iter_name = make_iter_name(iter_index)#iter_index form ii 迭代轮次
    work_path = os.path.join(iter_name, model_devi_name) #工作路径=迭代轮次+devi name
    assert os.path.isdir(work_path) #判断work_path是否是一个准确的目录

    all_task = glob.glob(os.path.join(work_path, "task.*")) #获取work_path下所有task文件夹目录
    all_task.sort()# 排序
    fp = open(os.path.join(work_path, "cur_job.json"), "r")#打开work_path下的cur_job.json赋值给fp
    cur_job = json.load(fp) #cur_job接收json内容 里面的内容是指定model_devi结构和温度的json，不知道有什么用

    run_tasks_ = all_task# work_path下所有task文件夹目录给到run_tasks_
    # for ii in all_task:
    ...
    run_tasks = [os.path.basename(ii) for ii in run_tasks_]#取出文件名task.* list
    # dlog.info("all_task is ", all_task)
    # dlog.info("run_tasks in run_model_deviation",run_tasks_)
    all_models = glob.glob(os.path.join(work_path, "*.pb"))# 获取所有.pb文件路径，文件应该是在pre_model devi中准备好的
    model_names = [os.path.basename(ii) for ii in all_models]#取出基本文件名，作为submission的common model

    model_devi_engine = jdata.get("model_devi_engine", "lammps")#从param中读取md运行软件，若没有则默认lammps
    if model_devi_engine == "lammps":#定义submission参数
        command = (
        ...
        command = "/bin/sh -c '%s'" % command
        commands = [command] #放入列表commands

        forward_files = ["conf.lmp", "input.lammps"]#上传文件默认
        backward_files = ["dev_neu.out", "dev_red.out","model_devi.log"] #下载文件默认
        if model_devi_merge_traj:#默认不开启
            backward_files += ["all.lammpstrj"]
        else:#
            forward_files += ["traj"]
            backward_files += ["traj"]#上传下载轨迹文件？实际文件夹中没看到
    ...
    eta_list = [str(item) for item in eta]
    user_forward_files = mdata.get("model_devi" + "_user_forward_files", [])#读取用户是否有额外的input文件，默认为空
    forward_files += [os.path.basename(file) for file in user_forward_files]
    backward_files += mdata.get("model_devi" + "_user_backward_files", [])#读取用户是否有额外的output文件
    api_version = mdata.get("api_version", "1.0")
    if len(run_tasks) == 0:#pre环节中没有创建文件夹
        raise RuntimeError(
    ...
    if Version(api_version) < Version("1.0"):
    ...
    elif Version(api_version) >= Version("1.0"):#1.x提交任务
        for task_path in all_task:
            submission = make_submission(
                ...
                work_path=task_path,
                run_tasks=eta_list,
                group_size=1,#直接设置为1吧
                forward_common_files=model_names,#跑 modeldevi需要的 models
                forward_files=forward_files,
                backward_files=backward_files,
...
def run_model_devi(iter_index, jdata, mdata):
    ...
    model_devi_engine = jdata.get("model_devi_engine", "lammps") #若不存在测默认使用lammps
    if model_devi_engine != "calypso":
        run_md_model_devi(iter_index, jdata, mdata)  #lammps model_devi的运行入口
    else:
...
def _read_model_devi_file(
    ...
    model_devi_neu = np.loadtxt(os.path.join(eta_path, "dev_neu.out"))
    if model_devi_f_avg_relative:
        if model_devi_merge_traj is True:
            all_traj = os.path.join(eta_path, "all.lammpstrj")
            all_f = get_all_dumped_forces(all_traj)
        else:
            trajs = glob.glob(os.path.join(eta_path, "traj", "*.lammpstrj"))
            all_f = []
            for ii in trajs:
        ...
        model_devi_neu[:, 4:7] = model_devi_neu[:, 4:7] / avg_f
        np.savetxt(
            os.path.join(eta_path, "model_devi_avgf.out"), model_devi_neu, fmt="%16.6e"
        )
    model_devi_red = np.loadtxt(os.path.join(eta_path, "dev_red.out"))
    if model_devi_f_avg_relative:
        if model_devi_merge_traj is True:
            all_traj = os.path.join(eta_path, "all.lammpstrj")
            all_f = get_all_dumped_forces(all_traj)
        ...
        all_f = all_f.reshape([-1, 3])
        avg_f = np.sqrt(np.average(np.sum(np.square(all_f), axis=1)))
        model_devi_red[:, 4:7] = model_devi_red[:, 4:7] / avg_f
        np.savetxt(
            os.path.join(eta_path, "model_devi_avgf.out"), model_devi_red, fmt="%16.6e"
        )
    return model_devi_neu,model_devi_red #平均力差以及所有轨迹信息
...
def _select_by_model_devi_standard(
    ...
    if detailed_report_make_fp:
        fp_rest_accurate_neu = []
        fp_rest_accurate_red = []
        fp_rest_failed_neu = []
        fp_rest_failed_red = []
    cc = 0
    counter_neu = Counter()#计数字典
    counter_neu["candidate"] = 0
    counter_neu["failed"] = 0
    counter_neu["accurate"] = 0
    counter_red = Counter()#计数字典
    counter_red["candidate"] = 0
    counter_red["failed"] = 0
    counter_red["accurate"] = 0
    num = np.linspace(0, 1, 5)
    eta_list = [str(item) for item in num]
    for tt in modd_system_task:
        for eta in eta_list:
            eta_paths = os.path.join(tt,eta)
        
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                all_conf_neu,all_conf_red = _read_model_devi_file(
                    eta_paths, model_devi_f_avg_relative, model_devi_merge_traj
                )#tt=task_path,后面两个参数默认无，all_conf是平均力差以及所有轨迹信息，id信息

                if all_conf_neu.shape == (7,):#展平
                    all_conf_neu = all_conf_neu.reshape(1, all_conf_neu.shape[0])

                if all_conf_red.shape == (7,):
                    all_conf_red = all_conf_red.reshape(1, all_conf_red.shape[0])
                
                for ii in range(all_conf_neu.shape[0]):#用ii遍历每个结构
                    if all_conf_neu[ii][0] < model_devi_skip:
                        continue
                    cc = int(all_conf_neu[ii][0])#结构标签
                    if cluster_cutoff is None:
                        if model_devi_engine == "calypso":
                            if float(all_conf_neu[ii][-1]) <= float(min_dis):
                                if detailed_report_make_fp:
                                    fp_rest_failed_neu.append([tt,eta_paths,cc])
                                counter_neu["failed"] += 1
                                continue
                        if (
                            all_conf_neu[ii][1] < v_trust_hi and all_conf_neu[ii][1] >= v_trust_lo#判断结构v
                        ) or (
                            all_conf_neu[ii][4] < f_trust_hi and all_conf_neu[ii][4] >= f_trust_lo#判断结构力
                        ):
                            fp_candidate_neu.append([tt,eta_paths,cc])
                            counter_neu["candidate"] += 1 #计数并加入列表，列表包括了，路径和轨迹id
                        elif (all_conf_neu[ii][1] >= v_trust_hi) or (
                            all_conf_neu[ii][4] >= f_trust_hi
                        ):
                            if detailed_report_make_fp:
                                fp_rest_failed_neu.append([tt,eta_paths,cc])
                            counter_neu["failed"] += 1
                        elif all_conf_neu[ii][1] < v_trust_lo and all_conf_neu[ii][4] < f_trust_lo:
                            if detailed_report_make_fp:
                                fp_rest_accurate_neu.append([tt,eta_paths,cc])
                            counter_neu["accurate"] += 1
                        else:
                            if model_devi_engine == "calypso":
                                dlog.info(
                                    "ase opt traj %s frame %d with f devi %f does not belong to either accurate, candidiate and failed "
                                    % (tt, ii, all_conf_neu[ii][4])
                                )
                            else:
                                raise RuntimeError(
                    ...
                    else:#cluster_cutoff ture
                        raise ValueError("cluster_cutoff should be None")
                    
                for ii in range(all_conf_red.shape[0]):#用ii遍历每个结构
                    if all_conf_red[ii][0] < model_devi_skip:
                        continue
                    cc = int(all_conf_red[ii][0])#结构标签
                    if cluster_cutoff is None:
                        if model_devi_engine == "calypso":
                            if float(all_conf_red[ii][-1]) <= float(min_dis):
                                if detailed_report_make_fp:
                                    fp_rest_failed_red.append([tt,eta_paths,cc])
                                counter_red["failed"] += 1
                                continue
                        if (
                            all_conf_red[ii][1] < v_trust_hi and all_conf_red[ii][1] >= v_trust_lo#判断结构v
                        ) or (
                            all_conf_red[ii][4] < f_trust_hi and all_conf_red[ii][4] >= f_trust_lo#判断结构力
                        ):
                            fp_candidate_red.append([tt,eta_paths,cc])
                            counter_red["candidate"] += 1 #计数并加入列表，列表包括了，路径和轨迹id
                        elif (all_conf_red[ii][1] >= v_trust_hi) or (
                            all_conf_red[ii][4] >= f_trust_hi
                        ):
                            if detailed_report_make_fp:
                                fp_rest_failed_red.append([tt,eta_paths,cc])
                            counter_red["failed"] += 1
                        elif all_conf_red[ii][1] < v_trust_lo and all_conf_red[ii][4] < f_trust_lo:
                            if detailed_report_make_fp:
                                fp_rest_accurate_red.append([tt,eta_paths,cc])
                            counter_red["accurate"] += 1
                        else:
                            if model_devi_engine == "calypso":
                                dlog.info(
                                    "ase opt traj %s frame %d with f devi %f does not belong to either accurate, candidiate and failed "
                                    % (tt, ii, all_conf_red[ii][4])
                                )
                            else:
                                raise RuntimeError(
                                    "md traj %s frame %d with f devi %f does not belong to either accurate, candidiate and failed, it should not happen"
    ...
    return fp_rest_accurate_neu, fp_candidate_neu, fp_rest_failed_neu,fp_rest_accurate_red, fp_candidate_red, fp_rest_failed_red, counter_neu,counter_red#返回计数以及轨迹结构列表
...
def _make_fp_vasp_inner(
    ...
    system_index = []
    for ii in modd_task:
        system_index.append(os.path.basename(ii).split(".")[1])#轮次数，eg:000

    set_tmp = set(system_index)
    system_index = list(set_tmp)
    system_index.sort()#轮次数，eg:000
    ...
    def _trust_limitation_check(sys_idx, lim):#这里莫名其妙接了个内部函数导致连贯性出问题，注意
    ...
    for ss in system_index:#task编号中间，000，既迭代轮次
        ...
        if model_devi_engine in ("lammps", "gromacs", "calypso"):
            ...
            v_trust_hi_sys = _trust_limitation_check(int(ss), v_trust_hi)#获得判断标准

            # assumed e -> v
            if not model_devi_adapt_trust_lo:#运行这一步，获取候选者列表
                (
                    fp_rest_accurate_neu,
                    fp_candidate_neu,
                    fp_rest_failed_neu,
        ...
        # print a report
        fp_neu_sum = sum(counter_neu.values())
        fp_red_sum = sum(counter_red.values())

        if fp_neu_sum == 0:
            dlog.info(
                "system {0:s} has no fp task, maybe the model devi in neu is nan %".format(ss)
            )
            continue
        if fp_red_sum == 0:
            dlog.info(
                "system {0:s} has no fp task, maybe the model devi in red is nan %".format(ss)
            )
            continue
        for cc_key, cc_value in counter_neu.items():
            dlog.info(
                "neu system {0:s} {1:9s} : {2:6d} in {3:6d} {4:6.2f} %".format(
                    ss, cc_key, cc_value, fp_neu_sum, cc_value / fp_neu_sum * 100
                )
            )
        for cc_key, cc_value in counter_red.items():
            dlog.info(
                "red system {0:s} {1:9s} : {2:6d} in {3:6d} {4:6.2f} %".format(
                    ss, cc_key, cc_value, fp_red_sum, cc_value / fp_red_sum * 100
                )
            )

        random.shuffle(fp_candidate_neu)
        random.shuffle(fp_candidate_red)

        if detailed_report_make_fp:
            random.shuffle(fp_rest_failed_neu)
            random.shuffle(fp_rest_failed_red)
            random.shuffle(fp_rest_accurate_red)
            random.shuffle(fp_rest_accurate_neu)
            with open(
                os.path.join(work_path, "candidate_neu.shuffled.%s.out" % ss), "w"
            ) as fp:
                for ii in fp_candidate_neu:
                    fp.write(" ".join([str(nn) for nn in ii]) + "\n")
            with open(
                os.path.join(work_path, "candidate_red.shuffled.%s.out" % ss), "w"
            ) as fp:
                for ii in fp_candidate_red:
                    fp.write(" ".join([str(nn) for nn in ii]) + "\n")

            with open(
                os.path.join(work_path, "rest_accurate_neu.shuffled.%s.out" % ss), "w"
            ) as fp:
                for ii in fp_rest_accurate_neu:
                    fp.write(" ".join([str(nn) for nn in ii]) + "\n")
            with open(
                os.path.join(work_path, "rest_failed_neu.shuffled.%s.out" % ss), "w"
            ) as fp:
                for ii in fp_rest_failed_neu:
                    fp.write(" ".join([str(nn) for nn in ii]) + "\n")

        # set number of tasks
        accurate_ratio_neu = float(counter_neu["accurate"]) / float(fp_neu_sum)
        accurate_ratio_red = float(counter_red["accurate"]) / float(fp_red_sum)
        fp_accurate_threshold = jdata.get("fp_accurate_threshold", 1) #默认为1
        ...
        if accurate_ratio_neu < fp_accurate_soft_threshold:
            this_fp_task_max = fp_task_max
        elif (
            accurate_ratio_neu >= fp_accurate_soft_threshold
            and accurate_ratio_neu < fp_accurate_threshold
        ):
            this_fp_task_max = int(
                fp_task_max
                * (accurate_ratio_neu - fp_accurate_threshold)
                / (fp_accurate_soft_threshold - fp_accurate_threshold)
            )
        else:
            this_fp_task_max = 0
        ...
        # # ----------------------------------------------------------------------------
        numb_task_neu = min(this_fp_task_max, len(fp_candidate_neu))#numb_task确定提交几个fp task
        if numb_task_neu < fp_task_min:
            numb_task_neu = 0

        #定义red_fp task数
        if accurate_ratio_red < fp_accurate_soft_threshold:
            this_fp_task_max = fp_task_max
        elif (
            accurate_ratio_red >= fp_accurate_soft_threshold
            and accurate_ratio_red < fp_accurate_threshold
        ):
            this_fp_task_max = int(
                fp_task_max
                * (accurate_ratio_red - fp_accurate_threshold)
                / (fp_accurate_soft_threshold - fp_accurate_threshold)
            )
        else:
            this_fp_task_max = 0

        numb_task_red = min(this_fp_task_max, len(fp_candidate_red))#numb_task确定提交几个fp task
        if numb_task_red < fp_task_min:
            numb_task_red = 0
        ...
        # # ----------------------------------------------------------------------------
        dlog.info(
            "neu system {0:s} accurate_ratio: {1:8.4f}    thresholds: {2:6.4f} and {3:6.4f}   eff. task min and max {4:4d} {5:4d}   number of fp tasks: {6:6d}".format(
                ss,
                accurate_ratio_neu,
        ...
        dlog.info(
            "red system {0:s} accurate_ratio: {1:8.4f}    thresholds: {2:6.4f} and {3:6.4f}   eff. task min and max {4:4d} {5:4d}   number of fp tasks: {6:6d}".format(
                ss,
                accurate_ratio_red,
                fp_accurate_soft_threshold,
                fp_accurate_threshold,
        ...
        count_bad_box = 0
        count_bad_cluster = 0
        #modified by LJS  分别取max task的结构出来
        fp_candidate_neu = sorted(fp_candidate_neu[:numb_task_neu])
        fp_candidate_red = sorted(fp_candidate_red[:numb_task_neu])

        fp_neu_tasks = [] #task 路径
        fp_red_tasks = []

###这里lammmp获取的candidate作为输入 fp
        #获取neu候选结构，软链，并创建fp task
        for cc in range(numb_task_neu):
            tt = fp_candidate_neu[cc][0]#tt=task_path
            eta_path = fp_candidate_neu[cc][1] # eta path
            ii = fp_candidate_neu[cc][2]#confs id
            ss = os.path.basename(tt).split(".")[1] #迭代轮次
            conf_name_neu = os.path.join(eta_path, "traj") #获取包含candidate的跑md的traj文件 路径
            conf_sys = None
            if model_devi_engine == "lammps":
                if model_devi_merge_traj:#无
                    conf_sys = all_sys[int(os.path.basename(tt).split(".")[-1])][
                        int(int(ii) / trj_freq)
                    ]
                else:
                    conf_name_neu = os.path.join(conf_name_neu, str(ii) + ".lammpstrj") #一个一个结构往外取，conf_name candidate,str(ii) + ".lammpstrj"文件名
                ffmt = "lammps/dump"
            elif model_devi_engine == "gromacs":
                ...
                raise RuntimeError("unknown model_devi engine", model_devi_engine)
            conf_name_neu = os.path.abspath(conf_name_neu)  #获得结构的绝对路径
            ...
            if model_devi_engine != "calypso":
                # link job.json
                job_name = os.path.join(eta_path, "job.json") #modified
                job_name = os.path.abspath(job_name)

            if cluster_cutoff is not None:
                # take clusters
                jj = fp_candidate[cc][3]
            ...
            fp_neu_task_name = make_fp_neu_task_name(int(ss), cc)
            fp_neu_task_path = os.path.join(work_path, fp_neu_task_name)
            create_path(fp_neu_task_path)#创建文件夹 neu 与 red 需要做一定区分
            fp_neu_tasks.append(fp_neu_task_path)
            if charges_map:
                charges_recorder.append(charges_map[int(ss)])
            cwd = os.getcwd()
            os.chdir(fp_neu_task_path)
            numb_task = 0
            if cluster_cutoff is None:
                if model_devi_engine == "lammps":
                    if model_devi_merge_traj:
                        ...
                        os.symlink(os.path.relpath(conf_name_neu), "conf.dump")#轨迹结构链接为dump文件
            ...
            for pair in fp_link_files:#为空跳过
                os.symlink(pair[0], pair[1])
            os.chdir(cwd) #路径还原回来
        
        #获取red候选结构,进行软链，并创建fp task
        for cc in range(numb_task_red):
            tt = fp_candidate_red[cc][0]#tt=task_path
            eta_path = fp_candidate_red[cc][1] # eta path
            ii = fp_candidate_red[cc][2]#confs id
            ss = os.path.basename(tt).split(".")[1] #迭代轮次
            conf_name_red = os.path.join(eta_path, "traj") #获取包含candidate的跑md的traj文件 路径
            conf_sys = None
            if model_devi_engine == "lammps":
                if model_devi_merge_traj:#无
                    conf_sys = all_sys[int(os.path.basename(tt).split(".")[-1])][
                    ...
                    conf_name_red = os.path.join(conf_name_red, str(ii) + ".lammpstrj") #一个一个结构往外取，conf_name candidate,str(ii) + ".lammpstrj"文件名
                ffmt = "lammps/dump"
            else:
                raise RuntimeError("unknown model_devi engine", model_devi_engine)
            conf_name_red = os.path.abspath(conf_name_red)  #获得结构的绝对路径
            if skip_bad_box is not None:
                skip = check_bad_box(conf_name, skip_bad_box, fmt=ffmt)
                if skip:
                    count_bad_box += 1
            ...
            fp_red_task_name = make_fp_red_task_name(int(ss), cc)
            fp_red_task_path = os.path.join(work_path, fp_red_task_name)
            create_path(fp_red_task_path)#创建文件夹 neu 与 red 需要做一定区分
            fp_red_tasks.append(fp_red_task_path)
            if charges_map:
                charges_recorder.append(charges_map[int(ss)])
            cwd = os.getcwd()
            os.chdir(fp_red_task_path) #进入路径
            if cluster_cutoff is None:
                if model_devi_engine == "lammps":
                    if model_devi_merge_traj:
                        conf_sys.to("lammps/lmp", "conf.dump")
                    else:
                        os.symlink(os.path.relpath(conf_name_red), "conf.dump")#轨迹结构链接为dump文件
                    os.symlink(os.path.relpath(job_name), "job.json")
                elif model_devi_engine == "gromacs":
                    os.symlink(os.path.relpath(conf_name), "conf.dump")
                    os.symlink(os.path.relpath(job_name), "job.json")
    ...
    if cluster_cutoff is None:
        fp_all_tasks = fp_neu_tasks + fp_red_tasks
        cwd = os.getcwd()
        for idx, task in enumerate(fp_all_tasks):
            os.chdir(task)
            if model_devi_engine == "lammps":
                sys = None
                if model_devi_merge_traj:
    ...
    return fp_neu_tasks,fp_red_tasks # 返回task路径列表
...
#以上代码用于给_make_fp_vasp_inner函数配置传入参数，获得结果后函数会用这个结果作为输出再返回
#---------------------------------分割线-------------------
    # make configs
    #modified by LJS _make_fp_vasp_inner函数会创建task文件,同时将candidate软链建立完毕，task数量最大数量需要在配置文档中改
    fp_neu_tasks,fp_red_tasks = _make_fp_vasp_inner(
        iter_index,
        modd_path,
        work_path, #iter/fp
        model_devi_skip,
    ...
    return fp_neu_tasks,fp_red_tasks  #fp_task的路径列表
...
def make_fp_cp2k(iter_index, jdata):
    # make config
    fp_neu_tasks,fp_red_tasks = _make_fp_vasp_configs(iter_index, jdata) #fp_task的路径列表
    if len(fp_neu_tasks) == 0 & len(fp_red_tasks) == 0:
        return
    # make cp2k input
    iter_name = make_iter_name(iter_index)
    work_path = os.path.join(iter_name, fp_name)
    ...
    elif "neu_external_input_path" in jdata.keys():#我用这个入口进行输入
        fp_params = None
        #exinput_path = os.path.abspath(jdata["external_input_path"])
        neu_exinput_path = os.path.abspath(jdata["neu_external_input_path"])
        red_exinput_path = os.path.abspath(jdata["red_external_input_path"])
    else:
        fp_params = jdata["fp_params"]
    #配置neu fp task的输入文件
    cwd = os.getcwd()
    for ii in fp_neu_tasks:
        os.chdir(ii)
        sys_data = dpdata.System("POSCAR").data
        ...
        if fp_params:
            ...
            cp2k_input = make_cp2k_input_from_external(sys_data, neu_exinput_path)
    ...
    for ii in fp_red_tasks:
        ...
        if fp_params:
            ...
            # else read from user input
            cp2k_input = make_cp2k_input_from_external(sys_data, red_exinput_path)
        with open("input.inp", "w") as fp:
            fp.write(cp2k_input)
            fp.close()
        # make coord.xyz used by cp2k for every task
...
def run_fp_inner(
    ...
    iter_name = make_iter_name(iter_index)
    work_path = os.path.join(iter_name, fp_name) #neu与red统一work_path

    fp_tasks = glob.glob(os.path.join(work_path, "task*")) #包含neu和red所有路径的列表
    fp_tasks.sort()
    if len(fp_tasks) == 0:
        return
    ...
    run_tasks = [os.path.basename(ii) for ii in fp_run_tasks] #获取fp task的basename列表 
    ...
    if Version(api_version) < Version("1.0"):
        ...
        #make_submission函数的优势在于，不需要转换成task 再将task list ,传入submission，可以直接将task的basename list 转换为task list 传入submission并实例化
        submission = make_submission(
            mdata["fp_machine"],
            mdata["fp_resources"],
            commands=[fp_command],
...
def run_fp(iter_index, jdata, mdata):
    ...
    if fp_style == "vasp":
        ...
        run_fp_inner(
            ...
            backward_files,
            _vasp_check_fin,
            forward_common_files=forward_common_files,#fp时这个参数为空
        )
    elif fp_style == "pwscf":
...
def post_fp_cp2k(iter_index, jdata, rfailed=None):
    ...
    ratio_failed = rfailed if rfailed else jdata.get("ratio_failed", 0.10) # 失败率为0.1
    model_devi_jobs = jdata["model_devi_jobs"] #结构文件id,和输入参数字典
    assert iter_index < len(model_devi_jobs) 
    ...
    #先处理neu labeling数据
    for ss in system_index:
        sys_output = glob.glob(os.path.join(work_path, "task_neu.%s.*/output" % ss))
        sys_output.sort()
        ...
        icount += len(all_sys)
        if all_sys is not None:
            sys_data_path = os.path.join(work_path, "data_neu.%s" % ss) #将labeling的数据放入fp/下的data 文件夹， 后续放入training dataset
            os.makedirs(sys_data_path)
            all_sys.to_deepmd_raw(sys_data_path)
            all_sys.to_deepmd_npy(sys_data_path, set_size=len(sys_output))

    #再处理red labeling数据
    for ss in system_index:
        sys_output = glob.glob(os.path.join(work_path, "task_red.%s.*/output" % ss))
        sys_output.sort()
        tcount += len(sys_output)
        all_sys = None
        for oo in sys_output:
        ...
        icount += len(all_sys)
        if all_sys is not None:
            sys_data_path = os.path.join(work_path, "data_red.%s" % ss) #将labeling的数据放入fp/下的data 文件夹， 后续放入training dataset
            os.makedirs(sys_data_path)
            all_sys.to_deepmd_raw(sys_data_path)
            all_sys.to_deepmd_npy(sys_data_path, set_size=len(sys_output))
    if tcount == 0:
